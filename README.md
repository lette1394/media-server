...
클라이언트는 스토리지 내부 스페이스에 파일을 업로드 할 수 있다. 파일은 업로드가 완료된 이후 스페이스, 파일 키, 파일 크기, 파일 종류(text/image/video/audio/binary), 업로드 시간을 알고 있어야 한다. 이를 파일의 메타 정보라고 부른다. 파일은 네트워크 문제 혹은 성능 개선을 위해 여러 조각으로 나눠서 업로드 될 수 있다. 업로드가 예상치 못하게 중단되는 경우 클라이언트는 마지막으로 파일이 업로드 된 부분부터 업로드를 이어서 할 수 있다. 클라이언트는 스페이스 구분 없이 마구잡이로 업로드를 할 수 없으며, 요청 파일의 메타 정보 또는 요청에 같이 포함된 추가 정보를 가지고 해당 스페이스가 결정할 수 있다. 거부된 파일은 저장되지 않으며 업로드 된 데이터는 단순히 버려진다. 스페이스는 파일 키:A에 대한 업로드가 진행 중에 새로운 파일 키:A에 대한 업로드 요청이 들어오면, 이전 요청을 취소하고 새로운 업로드를 수행할 수 있다. 이는 스페이스마다 다르다. 미디어 파일은 업로드 완료 이후에 인코딩 되어 복수개의 파생 파일이 생길 수 있다. (원본 파일 + 인코딩 파일), (인코딩 파일), (원본 파일 + 인코딩 파일1 + 인코딩 파일2 + ...) 

클라이언트는 다른 스페이스에서 내 스페이스로 파일을 복사해 올 수 있으며 이 때 다른 스페이스가 요청을 거부할 수 있다. 

스페이스 관리자(administrator)는 클라이언트의 파일 연산을 거절할 수 있다. 거절하는 이유는 관리자마다, 파일마다 각각 다르다. 일반적으로 공개 네트워크에 위치한 클라이언트가 개인정보가 포함된 파일에 대해 접근하려고 할 때 관리자는 이를 제한할 수 있다. 또한 관리자는 자신이 관리하는 스페이스에 어떤 일이 일어나는지 알람을 받을 수 있다. 파일 연산 중에 관리자는 클라이언트에게 추가적인 정보를 주거나 클라이언트로부터 정보를 받아서 파일 연산에 필요한 데이터를 스토리지에 알려줄 수 있다.   
 
 
빠른 다운로드 속도를 위해 스토리지는 캐시된 파일을 내려줄 수 있다. 클라이언트는 기존 파일을 덮어쓸 때 캐시된 파일에 대한 업데이트 여부를 결정할 수 있다. 

클라이언트는 파일을 업로드한 삭제할 수 있다. 파일은 바로 삭제되거나, 생성한지 X일 후에 삭제되거나, 마지막 접근 Y일 이후에 삭제될 수 있다. 
