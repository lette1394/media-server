...
미디어 파일은 업로드 완료 이후에 후처리를 통해 인코딩되어 복수개의 파생 파일이 생길 수 있다. (원본 파일 + 인코딩 파일), (인코딩 파일), (원본 파일 + 인코딩 파일1 + 인코딩 파일2 + ...) 어떤 방식으로 인코딩 할지는 스페이스에서 결정하거나 스페이스 관리자가 개별 파일 연산마다 결정할 수 있다. 파일의 후처리가 완료된 이후에 원본에 대한 삭제를 수행하거나 일정 기간 이후에 삭제할 수 있다. 이 동작 역시 스페이스 관리자가 개별적으로 결정하거나 스페이스에서 일괄적으로 처리할 수 있다. 

클라이언트는 다른 스페이스에서 내 스페이스로 파일을 복사해 올 수 있으며 이 때 다른 스페이스가 요청을 거부할 수 있다. 스페이스와 파일 구성에 따라 실제로 데이터를 복사하지 않을 수 있으며, 단순히 소스 파일에 대한 참조만 가지고 마치 복사가 수행된 듯 연산을 완료할 수 있다. 스토리지 성능 최적화를 위해 특정 참조 횟수가 기준치를 넘는 경우 새로운 복사 대상이 생성되며, 이후 참조 복사는 새롭게 생성된 복사 대상을 바라본다. 
