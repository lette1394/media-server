...
유저는 스토리지 안의 스페이스에 파일을 업로드 할 수 있다. 파일은 업로드가 완료된 이후 스페이스, 파일 키, 파일 크기, 파일 종류(text/image/video/audio/binary), 업로드 시간을 알아야 한다. 이를 파일의 메타 정보라고 부른다. 파일은 네트워크 문제 혹은 성능 개선을 위해 여러 조각으로 나눠서 업로드 될 수 있다. 업로드가 예상치 못하게 중단되는 경우 유저는 마지막으로 파일이 업로드 된 부분부터 업로드를 이어서 할 수 있다. 모든 업로드 요청이 허용되지 않으며, 어떤 요청이 거절되는지 유저가 결정할 수 있다. [유저: 여기서 뭔가 의미가 다르다. 이 '유저'는 실제로 파일을 올리는 '유저'가 아니다.]는 파일의 메타 정보를 가지고 혹은 요청에 같이 포함된 추가 정보를 가지고 업로드를 제한할 수 있다. 제한 된 파일 요청은 저장되지 않으며 업로드 된 데이터는 단순히 버려진다. 
